 ...
 */
public class Fantasma extends Pacman {
 private boolean activa=true;
        private static int angulo=100;

    
    
    public Fantasma(float vX, float vY, int radi, float x, float y, int colorR, int colorG, int colorB) {
        super(vX, vY, radi, x, y, colorR, colorG, colorB);
    }
    
    public Fantasma(Joc joc, int margeX, int margeY) {
    ...
    public void dibuixa(Joc joc){
       ...
       if(this.isActiva()==true) {
               ...
               joc.fill(getColorR(), getColorG(), getColorB());
                ...
                joc.fill(0);
                joc.ellipse(x-(radi/2),y-(radi/2), 2*(radi/6), 2*(radi/6));
                joc.ellipse(x+(radi/2),y-(radi/2), 2*(radi/6), 2*(radi/6));
                joc.ellipse(x,y+(radi/2), 2*(radi/3), 2*(radi/3));
